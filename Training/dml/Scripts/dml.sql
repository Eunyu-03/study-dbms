...
values('호돌이', '호랑이', 205.42, 115.23, 1);
...
values('abd', '1234', '10', '홍길동');

insert into tbl_member
(member_id, member_password, member_age, member_name)
values('efg', '5678', '20', '정은유');

insert into tbl_member
(member_id, member_password, member_age, member_name)
values('hig', '9101', '30', '띠용이');
...
select *
from tbl_like
where like_receiver=1 and like_sender =2;
...
select m.member_age, m.member_name, p.post_title, p.post_content, p.id
from tbl_member m
join tbl_post p
...
values ("testgmail@gmail.com", "1234", "테스트", "서울시 강남구", '2000-12-04');
insert into tbl_user (user_email, user_password, user_name, user_address, user_birth)
values ("test2gmail@gmail.com", "1234", "테스트2", "서울시 강남구", str_to_date('2000년 12월 04일', '%Y년 %m월 %d일'));
...
values('휴대전화', 400000, 30), ('게임기v1', 150000, 5), ('게임기 v2', 200000, 6), ('마우스', 55000, 52);

select  concat('상품명: ', product_name) 상품명 from tbl_product tp;

select * from tbl_order;

insert into tbl_order (user_id, product_id)
values(1, 3), (2, 3), (1, 1), (1, 2), (2, 1);

select date_format(order_date, '%Y년 %m월 %d일 %h시 %i분 %s초') from tbl_order;
...
select p.pet_ill_name 병명, avg(p.pet_age), max(o.owner_age)
from tbl_owner o
join tbl_pet p 
on p.owner_id =o.id 
...
select a.field_trip_id "체험학습 id", avg(c.child_age) "평균 나이"
from tbl_apply a
join tbl_child c 
on a.child_id =c.id 
group by a.field_trip_id ;

select kindergarten_id  "진행 유치원 id", count(id)
from tbl_field_trip 
group by kindergarten_id ;

select * from tbl_apply;

select kindergarten_id  "지원 유치원 id", count(a.child_id)
...
values 
   ('test@gmail.com', '1234', '테스트1', '서울시 강남구', 30),
   ('test2@gmail.com', '1234', '테스트2', '서울시 강남구', 20),
   ('test3@gmail.com', '1234', '테스트3', '서울시 강남구', 20),
   ('test4@gmail.com', '1234', '테스트4', '서울시 강남구', 40),
   ('test5@gmail.com', '1234', '테스트5', '서울시 강남구', 30),
   ('test6@gmail.com', '1234', '테스트6', '서울시 강남구', 30),
   ('test7@gmail.com', '1234', '테스트7', '서울시 강남구', 50);
...
/*20대 중에서 구매 가격이 10000원을 넘는 주문번호, 상품가격, 연령대 조회*/
...
where dd.product_price>100000 and 
dd.user_age between 20 and 29;
